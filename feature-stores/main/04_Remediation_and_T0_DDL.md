...
This DO-block:
- Computes max pipeline completion time (`img_max_done_at`) from all `*_done_at` columns on `ml.device_image_features_v1`
- Computes max asset timestamp (`img_max_asset_at`) from `"device".device_image_assets` (auto-discovers the best timestamp column)
