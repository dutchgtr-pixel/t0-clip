# Phase 5 Remediation — T0-safe Object Build (SQL)

This file contains the SQL definitions for the **T0-safe replacement objects** and the **certified survival feature-store entrypoint**.

**Important**:
- These statements are designed to be applied in the a Postgres database.
- They preserve legacy objects but create new `_t0_v1_*` and `_asof_*` objects.
...
## 1) Speed anchor (as-of) — replaces time-of-query speed anchor

### 1.1 Create `ml.tom_speed_anchor_asof_v1_mv`
This MV anchors all “recency weighting” and embargo logic to an explicit `anchor_day` rather than `CURRENT_DATE`.
...
## 2) TOM enriched speed (T0) — replaces legacy speed-enriched MV
...
## 3) TOM enriched AI clean (T0) — as-of windowed AI join
...
## 4) Geo mapping pinned (T0-safe dimension behavior)

### 4.1 Pin the current release_id into a stable view
```sql
DO $$
DECLARE rid bigint;
BEGIN
...
### 4.2 Build pinned geo dimension view (no dependency on ref.geo_mapping_current)
```sql
...
## 5) TOM read view for certified entrypoint (adds pinned geo)
```sql
...
## 6) Image feature store (T0) — SLA gated MV with img_within_sla

This DO-block:
- Computes max pipeline completion time (`img_max_done_at`) from all `*_done_at` columns on `ml.iphone_image_features_v1`
- Computes max asset timestamp (`img_max_asset_at`) from `"iPhone".iphone_image_assets` (auto-discovers the best timestamp column)
- Emits `img_within_sla` and NULLs feature columns when not within SLA.
...
## 7) Damage fusion scored (T0) — gated by img_within_sla
...
## 8) Device meta encoded (T0) — rewrite legacy viewdef to T0 upstreams
...
Then create the T0 MV keyed to the T0 base:
```sql
DO $$
DECLARE
  cols text;
...
## 9) Certified survival feature store entrypoint (T0) — with strict SLA gating
...
## 10) Legacy object deprecation (strongly recommended)
