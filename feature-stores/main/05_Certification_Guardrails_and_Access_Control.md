# Certification, Guardrails, and Access Control (SQL)

This file contains the **audit schema**, the **baseline artifacts**, the **certification assertions**, and the **training-time enforcement**.

---

## 1) Baseline tables

### 1.1 Viewdef baseline (closure)
```sql
...
Populate/update the baseline for the certified entrypoint closure:
```sql
...
### 1.2 Dataset hash baseline (dynamic falsification)
```sql
...
Insert/update baselines for multiple historical days:
```sql
WITH picked AS (
  WITH bounds AS (
...
## 3) Viewdef drift assertion (fast)
...
## 4) Certification guard functions

### 4.1 Basic registry freshness guard
```sql
...
### 4.2 Strict guard (freshness + viewdef drift)
```sql
...
### 4.3 Boolean wrapper for use in guarded view
```sql
...
## 5) Certification assertion + runner

### 5.1 Assertions (recommended canonical form)
This function is your “single point of truth” for what certification means.
...
### 5.2 Runner procedure (writes registry status)
```sql
...
Optional: also register the alias as certified:
```sql
INSERT INTO audit.t0_cert_registry(entrypoint,status,viewdef_objects,dataset_days,notes)
SELECT
...
## 7) Training access control (recommended minimum)
