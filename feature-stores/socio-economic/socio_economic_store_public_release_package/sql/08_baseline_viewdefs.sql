-- 08_baseline_viewdefs.sql
-- Populate viewdef SHA256 baselines for the certified entrypoint closure.

DELETE FROM audit.t0_viewdef_baseline
WHERE entrypoint = 'ml.socio_market_feature_store_t0_v1_v';

WITH RECURSIVE rels(oid) AS (
  SELECT 'ml.socio_market_feature_store_t0_v1_v'::regclass
  UNION
  SELECT c2.oid
  FROM rels r
...
defs AS (
  SELECT
    'ml.socio_market_feature_store_t0_v1_v'::text AS entrypoint,
