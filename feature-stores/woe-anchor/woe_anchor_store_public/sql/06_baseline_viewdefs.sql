-- 06_baseline_viewdefs.sql
-- Captures viewdef closure hashes for the entrypoint.

DELETE FROM audit.t0_viewdef_baseline
WHERE entrypoint = 'ml.woe_anchor_feature_store_t0_v1_v';

WITH RECURSIVE rels(oid) AS (
  SELECT 'ml.woe_anchor_feature_store_t0_v1_v'::regclass
  UNION
  SELECT c2.oid
  FROM rels r
...
defs AS (
  SELECT
    'ml.woe_anchor_feature_store_t0_v1_v'::text AS entrypoint,
