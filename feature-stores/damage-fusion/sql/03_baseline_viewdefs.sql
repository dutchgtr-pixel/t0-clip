-- ============================================================
-- Fusion Store — Baseline viewdef closure hashes (Gate A)
-- ============================================================

CREATE EXTENSION IF NOT EXISTS pgcrypto;
CREATE SCHEMA IF NOT EXISTS audit;

-- Baseline all VIEW/MATVIEW defs in the dependency closure.
DELETE FROM audit.t0_viewdef_baseline
WHERE entrypoint = 'ml.fusion_feature_store_t0_v1_v';

WITH RECURSIVE rels(oid) AS (
  SELECT 'ml.fusion_feature_store_t0_v1_v'::regclass
  UNION
  SELECT c2.oid
  FROM rels r
...
defs AS (
  SELECT
    'ml.fusion_feature_store_t0_v1_v'::text AS entrypoint,
