ml.device_image_features_unified_v1 — Unified Vision Feature Store (v1)
Purpose

ml.device_image_features_unified_v1 is the canonical listing-level “vision” feature store that unifies:
...
ml.device_image_accessory_features_v1

ml.device_image_color_features_v1

ml.device_image_damage_features_v1 (+ one computed structural mean)
...
Grain: one row per listing with at least one image in "device".device_image_assets.
...
1) "device".device_image_assets (image inventory + captions)
...
2) ml.device_image_features_v1 (per-image vision outputs, feature_version=1)
...
ml.device_image_damage_features_v1
...
New listings/images ingested into device_image_assets appear automatically.

New labeling writes to ml.device_image_features_v1 (accessory/color/damage scripts) flow through automatically.

Row-count behavior:

increases when the number of distinct (generation,listing_id) in device_image_assets increases.
...
These come from device_image_assets.
...
n_feat_rows_all (bigint): count of rows in ml.device_image_features_v1 for this listing
...
Unified includes the most important outputs from ml.device_image_accessory_features_v1.
...
Unified includes the listing-level color consensus outputs from ml.device_image_color_features_v1.
...
Unified includes summary and HQ-structural damage signals from ml.device_image_damage_features_v1, plus one computed mean.
...
row count equals number of distinct listings in device_image_assets
