...
"""Selected historical production model and numerical losses.

Definitions below are copied by AST-selected source spans, without changes to
function/class bodies. Data access, feature names, orchestration and private
configuration are deliberately outside this module. See provenance.json.
"""
...
class TrainConfig:
    ...
    use_legacy_img_vector: bool = True
    use_k8_vectors: bool = False
    k8_slots: int = 8
    k8_img_dim: int = 512
    k8_report_dim: int = 768
    k8_img_block_dropout: float = 0.0
    k8_report_block_dropout: float = 0.0
    text_block_dropout: float = 0.0
    pa_block_dropout: float = 0.0
    chan_block_dropout: float = 0.0
    ...
    tab_token_mode: str = "compact"  # feature | compact | pool
...
class K8SlotTokenizer(nn.Module):
    """Projects per-slot K8 vectors into one compact token per slot."""

    def __init__(self, d_in: int, d_model: int, n_slots: int, dropout: float, modality_id: int):
        super().__init__()
        self.d_in = int(d_in)
        self.d_model = int(d_model)
        self.n_slots = int(n_slots)
        self.proj = nn.Linear(self.d_in, self.d_model)
        self.slot_pos = nn.Parameter(torch.randn(self.n_slots, self.d_model) * 0.02)
        self.modality = nn.Parameter(torch.randn(1, self.d_model) * (0.02 + 0.01 * int(modality_id)))
        self.missing = nn.Parameter(torch.randn(1, self.d_model) * 0.02)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor, missing: Optional[torch.Tensor] = None) -> torch.Tensor:
        # x: (B, slots, dim); missing: (B, slots), 1 when unavailable.
        B, S, _ = x.shape
        if S != self.n_slots:
            raise ValueError(f"K8 slot tensor has {S} slots, expected {self.n_slots}")
        if missing is None:
            missing = torch.zeros((B, S), dtype=x.dtype, device=x.device)
        missing = missing.to(dtype=x.dtype, device=x.device).clamp(0.0, 1.0)
        x = x * (1.0 - missing).unsqueeze(-1)
        tok = self.proj(x)
        tok = tok + self.slot_pos.unsqueeze(0) + self.modality.view(1, 1, self.d_model)
        tok = tok + missing.unsqueeze(-1) * self.missing.view(1, 1, self.d_model)
...
class MultiModalSurvModel(nn.Module):
    def __init__(self, n_num: int, cat_cardinalities: List[int], cfg: TrainConfig):
        super().__init__()
        self.cfg = cfg
        self.use_legacy_img_vector = bool(getattr(cfg, "use_legacy_img_vector", True))
        self.use_k8_vectors = bool(getattr(cfg, "use_k8_vectors", False))
        self.k8_slots = int(getattr(cfg, "k8_slots", 8))
        self.text_block_dropout = float(getattr(cfg, "text_block_dropout", 0.0))
        self.k8_img_block_dropout = float(getattr(cfg, "k8_img_block_dropout", 0.0))
        self.k8_report_block_dropout = float(getattr(cfg, "k8_report_block_dropout", 0.0))
        self.pa_block_dropout = float(getattr(cfg, "pa_block_dropout", 0.0))
        self.chan_block_dropout = float(getattr(cfg, "chan_block_dropout", 0.0))
        self.pa_num_indices = list(getattr(cfg, "pa_num_indices", []) or [])
        self.pa_cat_indices = list(getattr(cfg, "pa_cat_indices", []) or [])
        self.chan_num_indices = list(getattr(cfg, "chan_num_indices", []) or [])
        self.chan_cat_indices = list(getattr(cfg, "chan_cat_indices", []) or [])
        ...
        if self.use_legacy_img_vector:
            self.img_tok = DenseTokenizer(512, cfg.d_model, cfg.img_tokens, cfg.dropout)
        else:
            self.img_tok = None
        if self.use_k8_vectors:
            self.k8_img_tok = K8SlotTokenizer(512, cfg.d_model, self.k8_slots, cfg.dropout, modality_id=1)
            self.k8_report_tok = K8SlotTokenizer(768, cfg.d_model, self.k8_slots, cfg.dropout, modality_id=2)
        else:
            self.k8_img_tok = None
            self.k8_report_tok = None

        self.tokens_total = tab_tokens + cfg.text_tokens
        if self.use_legacy_img_vector:
            self.tokens_total += cfg.img_tokens
        if self.use_k8_vectors:
            self.tokens_total += 2 * self.k8_slots
    ...
    def _drop_dense_block(self, x: torch.Tensor, missing: Optional[torch.Tensor], p: float) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        if (not self.training) or p <= 0.0 or x.shape[0] == 0:
            return x, missing
        keep = (torch.rand((x.shape[0], 1), device=x.device, dtype=x.dtype) >= float(p)).float()
        x = x * keep
        if missing is not None:
            missing = torch.maximum(missing.to(dtype=x.dtype), 1.0 - keep)
        return x, missing

    def _drop_slot_block(self, x: torch.Tensor, missing: Optional[torch.Tensor], p: float) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        if (not self.training) or p <= 0.0 or x.shape[0] == 0:
            return x, missing
        keep = (torch.rand((x.shape[0], 1, 1), device=x.device, dtype=x.dtype) >= float(p)).float()
        x = x * keep
        if missing is not None:
            keep2 = keep.squeeze(-1)
            missing = torch.maximum(missing.to(dtype=x.dtype), 1.0 - keep2)
        return x, missing

    def _drop_tabular_indices(self, x_num: torch.Tensor, x_cat: torch.Tensor, p: float, num_idx: List[int], cat_idx: List[int]) -> Tuple[torch.Tensor, torch.Tensor]:
        if (not self.training) or p <= 0.0 or x_num.shape[0] == 0:
            return x_num, x_cat
        if not num_idx and not cat_idx:
            return x_num, x_cat
        drop = (torch.rand((x_num.shape[0],), device=x_num.device) < float(p))
        if not torch.any(drop):
            return x_num, x_cat
        if num_idx:
            x_num = x_num.clone()
            rows = torch.where(drop)[0]
            cols = torch.as_tensor(num_idx, device=x_num.device, dtype=torch.long)
            x_num[rows[:, None], cols[None, :]] = 0.0
        if cat_idx and x_cat.numel() > 0:
            x_cat = x_cat.clone()
            rows = torch.where(drop)[0]
            cols = torch.as_tensor(cat_idx, device=x_cat.device, dtype=torch.long)
            x_cat[rows[:, None], cols[None, :]] = 0
        return x_num, x_cat
    ...
    def forward(
        ...
        x_num, x_cat = self._drop_tabular_indices(x_num, x_cat, self.pa_block_dropout, self.pa_num_indices, self.pa_cat_indices)
        x_num, x_cat = self._drop_tabular_indices(x_num, x_cat, self.chan_block_dropout, self.chan_num_indices, self.chan_cat_indices)
        x_text, m_text = self._drop_dense_block(x_text, m_text, self.text_block_dropout)
        tab_toks = self.tab_tok(x_num, x_cat)  # [B, T_tab, D]
        text_toks = self.text_tok(x_text)  # [B, T_text, D]
        toks = [tab_toks, text_toks]
        if self.use_legacy_img_vector and self.img_tok is not None:
            img_toks = self.img_tok(x_img)  # [B, T_img, D]
            toks.append(img_toks)
        if self.use_k8_vectors:
            if x_k8_img is None:
                x_k8_img = torch.zeros((x_text.shape[0], self.k8_slots, 512), device=x_text.device, dtype=x_text.dtype)
            if x_k8_report is None:
                x_k8_report = torch.zeros((x_text.shape[0], self.k8_slots, 768), device=x_text.device, dtype=x_text.dtype)
            if m_k8_img is None:
                m_k8_img = torch.ones((x_text.shape[0], self.k8_slots), device=x_text.device, dtype=x_text.dtype)
            if m_k8_report is None:
                m_k8_report = torch.ones((x_text.shape[0], self.k8_slots), device=x_text.device, dtype=x_text.dtype)
            x_k8_img, m_k8_img = self._drop_slot_block(x_k8_img, m_k8_img, self.k8_img_block_dropout)
            x_k8_report, m_k8_report = self._drop_slot_block(x_k8_report, m_k8_report, self.k8_report_block_dropout)
            toks.append(self.k8_img_tok(x_k8_img, m_k8_img))
            toks.append(self.k8_report_tok(x_k8_report, m_k8_report))
