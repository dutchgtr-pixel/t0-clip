...
def prcs_score(P: float, R: float, C: float, S: float, wP: float, wR: float, wC: float, wS: float) -> float:
    ...
    lp = float(_logit(np.array([P], dtype=float))[0])
...
def confusion_metrics_sold(p_fast_sold: np.ndarray, dur_sold: np.ndarray, thr: float, slow_h: float) -> Dict[str, float]:
...
def threshold_search_best_prcs_score_exact(
    ...
    for thr in thrs:
        ...
        if math.isnan(P) or math.isnan(R) or math.isnan(C) or math.isnan(S):
            continue
        if cm["bucket"] < int(min_bucket_total):
            continue
        if precision_min >= 0.0 and P < precision_min:
            continue
        if sac_max >= 0.0 and S > sac_max:
            continue

        score = prcs_score(P, R, C, S, wP=wP, wR=wR, wC=wC, wS=wS)
        if score > best_score:
