...
"""
observe_images_playwright.py (public-safe)
...
- No target-platform identifiers or selectors. All collection is generic and configurable.
...
- PG_SCHEMA: DB schema (default: device)
- LISTINGS_TABLE: listings table (default: device_listings)
- IMAGE_ASSETS_TABLE: image assets table (default: device_image_assets)
...
- LISTING_IMAGES_OBSERVED_AT_COLUMN: observe-stamp column (default: images_observed_at)
...
PG_SCHEMA = os.getenv("PG_SCHEMA", "device")
LISTINGS_TABLE = os.getenv("LISTINGS_TABLE", "device_listings")
IMAGE_ASSETS_TABLE = os.getenv("IMAGE_ASSETS_TABLE", "device_image_assets")
...
LISTING_IMAGES_OBSERVED_AT_COLUMN = os.getenv(
    "LISTING_IMAGES_OBSERVED_AT_COLUMN", "images_observed_at"
)
...
def get_pending_rows(limit: int) -> List[Tuple[int, int, str]]:
    ...
    q = sql.SQL(
        ...
        SELECT {gen_col}, {id_col}, {url_col}
          FROM {schema}.{tbl}
         WHERE {images_observed_at_col} IS NULL
        ...
        images_observed_at_col=_id(LISTING_IMAGES_OBSERVED_AT_COLUMN),
...
def mark_images_observed(generation: int, listing_id: int) -> None:
    """Stamp images_observed_at so we don't retry the listing in future runs."""
    conn = _conn()
    cur = conn.cursor()
    ...
    q = sql.SQL(
        """
        UPDATE {schema}.{tbl}
           SET {images_observed_at_col} = now()
...
# -----------------------------
# Observe listing images (generic)
# -----------------------------
...
def observe_listing_images(page, url: str, generation: int, listing_id: int) -> Tuple[int, bool]:
    """
    ...
    if status_code in (404, 410) or is_html_404(page):
        print(f"[OBSERVE] 404 listing_id={listing_id} url={url}")
        return 0, True
...
def run_batch(rows: Sequence[Tuple[int, int, str]]) -> None:
    ...
    with sync_playwright() as p:
        ...
        for gen, listing_id, url in rows:
            ...
            try:
                saved, is_404 = observe_listing_images(page, url, gen, listing_id)
                if saved > 0:
                    print(f"[OK] saved={saved} listing_id={listing_id}")
                    mark_images_observed(gen, listing_id)
                elif is_404:
                    print(f"[WARN] 404/no images listing_id={listing_id} (marking observed)")
                    mark_images_observed(gen, listing_id)
                else:
                    # No images found but not a 404. Mark observed to avoid infinite retries.
                    print(f"[WARN] no images extracted listing_id={listing_id} (marking observed)")
                    mark_images_observed(gen, listing_id)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--single-listing-id",
        type=int,
        help="Run observer only for this listing_id (manual test mode)",
    )
