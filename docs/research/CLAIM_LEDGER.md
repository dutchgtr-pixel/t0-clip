...
| C29 | All 911 retained frozen validation/evaluation masks exactly match the documented origin cutoff; event-window geometry explains the fast-event pattern | Fresh aggregate reconstruction and [formal mechanism](../../research/leakage/availability_mechanism.md) | Proof the historical mask was available at origin, invention of selection bias, or the model's causal dependence on the mask |
| C30 | An origin-admissible mask has strong apparent discrimination after event-window selection in an enumerated construction, but balanced accuracy 0.5 under the independent origin-selected control | [Executed constructed experiment](../../research/leakage/availability_mechanism_results.json), distinct from historical observations | Real-data predictive accuracy or a general instruction to remove every calendar feature |
| C31 | Seven saved Stage 0 policies share 523 records; all paired F1 intervals against mean-logit include zero | [Retained experiment audit](../../research/thesis_evidence/experimental_contributions.json) | Independent confirmation after method selection or reconstruction of the complete underlying seed-score matrices |
| C32 | On 413 matched Stage 1 records the hybrid has F1 0.875214 versus mean-logit 0.686567, with a conditional paired interval for the difference [0.144285, 0.235651] | Fresh paired reanalysis of retained decisions; all-positive reference and precision constraint also reported | Architecture-only effect, prospective transfer, selection-adjusted significance, or a result on the separate 964-row cohort |
| C33 | A documented context-source correction raises fixed-policy F1 from 0.3077 to 0.7077 on 441 scored records from a frozen 443-row cohort | Retained historical forensic report, weights/labels/threshold reported unchanged | Fresh paired replay: both original prediction vectors have not been recovered |
| C34 | A separate 587-row replay has base F1 0.370079 and meta F1 0.090909 | Fresh arithmetic from retained joined labels and predictions | Attribution solely to temporal decay, a direct contradiction of a different cohort's metric, or uniformly successful ensemble transfer |
