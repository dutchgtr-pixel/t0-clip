...
NEW_FIGURES = {
    ...
    "LEAKAGE_STUDY.md": [("r03_missingness_shortcut", "Lifecycle-dependent missingness in the earlier frozen validation and holdout cohorts. Every observed FAST72 positive belongs to the missing-pattern group. Numerators and denominators are shown; this demonstrates a shortcut opportunity, not a numerical causal estimate of metric inflation."), ("r08_tail_overlap", "Key overlap in a later rebuilt source: both exported tail cohorts are contained in its training table. This reconstruction does not establish that the original locked training matrix was identical."), ("r14_selection_geometry", "Algebraic selection geometry, with arbitrary time units. Event-window selection constrains origin plus duration to a diagonal band, turning a coverage cutoff into a duration proxy. Origin-window selection with complete follow-up does not create that relationship when origin and duration are independent. This schematic is distinct from the separately executed historical audit and constructed experiment.")],
    "12_original_contributions.md": [("r15_paired_ensembles", "New paired reanalysis of retained decisions, relative to each study's saved mean-logit policy. Stage 0 has 523 records and a 504-hour tail target; Stage 1 has 413 records and a 168-hour fast-event target. Intervals use 5,000 IID paired row resamples conditional on fitted models and policies; no selection, temporal-dependence or multiplicity correction is included. The tree stack is a meta-layer over neural scores, not a raw-feature tree baseline.")],
...
def cover(path):
    ...
    c.setFont("Helvetica",10);c.drawString(65,207,"Technical edition 0.5 / 22 September 2026")
...
The principal finding is an exactly reconstructed relationship between feature coverage and outcome-selected cohort geometry. All 911 frozen validation/evaluation masks match a documented origin cutoff; a formal derivation and an executed negative control explain why prediction-time admissibility alone does not prevent a cohort-induced duration shortcut. A separate historical fixed-policy source correction increased F1 from 0.3077 to 0.7077 on 441 scored records. The release connects these findings to source reconstruction, temporal contracts, portable model code, SQL checks and aggregate evidence, distinguishing newly recomputed results from documentary reports.

Retained exports verify 964 common records for the historical AFT/neural comparison: recomputed AFT F1 is 0.8560 and the associated neural meta-ensemble aggregate is 0.9209, with higher precision and lower recall. Separate paired reanalyses compare seven ensemble methods on 523 Stage 0 records and two methods on 413 Stage 1 records. The latter hybrid gains 0.1886 F1 over a selective mean-logit policy; its conditional paired interval is [0.1443,0.2357]. The analogous Stage 0 tree stack does not improve F1, and an unfavorable later replay is retained. The contribution is positioned against established leakage, temporal-database and multimodal-survival research. These completed retrospective studies and constructed mechanism tests have distinct scopes from future prospective validation.
...
""",encoding="utf-8")
    ...
    writer.add_metadata({"/Title":"MarketNeural: Learning Time in Moving Markets","/Author":"Ghaffar Masomi","/Subject":"Evidence-led research monograph, technical edition 0.5"})
